 ...
 */
var typeBool = function typeBool(bool, type) {
  switch (bool) {
    case undefined:
    case null:
...
var _setNull = function _setNull(viewmodel, val) {
  viewmodel.get(val).setValue(null);
};

var _getValue = function _getValue(viewmodel, val) {
  return viewmodel.get(val).getValue();
};

var createNumberList = function createNumberList(num) {
  return Array.from(Array(num), function (v, k) {
    return k;
  });
}; // 生成笛卡尔积数组
