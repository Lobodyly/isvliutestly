...
public class RuleEngineUtils {
    ...
    public static RuleContext prepareRuleContext(BaseReqDto queryParam) throws Exception {
        ...
        // 补充bustype参数source.billnum
        Object data = queryParam.getData();
        if (data != null) {
            JSONObject dataJson = JSONObject.parseObject(data.toString());
            if (dataJson != null && !dataJson.containsKey("billTypeId")) {
                dataJson.put("billTypeId", appsource + "." + uiMetaBaseInfo.getBillnum());
                queryParam.setData(dataJson.toString());
            }
        }
