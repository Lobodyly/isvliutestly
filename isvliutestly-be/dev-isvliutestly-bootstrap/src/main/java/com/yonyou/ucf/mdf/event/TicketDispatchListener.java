...
import com.yonyou.ucf.mdd.isv.sso.web.isv.PushAuthController;
...
public class TicketDispatchListener implements ISVEventListener<ISVEvent> {
    ...
    private final PushAuthController pushAuthController;

    @Override
    public boolean onEvent(ISVEventType type, ISVEvent event, EncryptionHolder holder) {
        com.yonyou.ucf.mdd.isv.sso.crypto.EncryptionHolder encryptionHolder = new com.yonyou.ucf.mdd.isv.sso.crypto.EncryptionHolder();
        BeanUtils.copyPropertiesIgnoresNull(holder, encryptionHolder);
        pushAuthController.authCallbackHandler(encryptionHolder);
        return true;
    }
